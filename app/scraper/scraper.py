...
IPHONE_FIREFOX_AGENT = ("Mozilla/5.0 (iPhone; CPU iPhone OS 14_7 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) FxiOS/131.0 Mobile/15E148 Safari/605.1.15")
...
async def scrape_website_async(url, max_wait_time=14000, check_interval=400, no_change_limit=3, change_limit=8):
    ...
    try:
        async with async_playwright() as playwright:
            ...
            await page.route("**/*", lambda route, request: asyncio.create_task(handle_request(route, request)))
            page.on("response", lambda response: script_capture_tasks.append(
                asyncio.create_task(capture_scripts_async(response, script_urls, script_contents))
            ))
